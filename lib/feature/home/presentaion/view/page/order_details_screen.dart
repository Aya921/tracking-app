...
//                             orderEntity: order,
//                             phoneNum: driver.contactInfo.phone,
//                             fromOrderDetails: true,
//                           ),
//
//
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  StreamSubscription? _orderSubscription; // 👈 أضفنا ده
  ...
  void initState() {
    ...
    // 👇 استماع مع حفظ الاشتراك
    _orderSubscription = getIt<HomeFirebaseService>()
        .getDataFromRemote(widget.orderId)
        .listen((data) {
      if (mounted) {
        setState(() {
          currentStep =
              _mapStateToStep(data.orderDeliveryStatus ?? 'waiting');
        });
      }
    });
  }
  ...
  @override
  void dispose() {
    // 👇 إلغاء الاشتراك عشان مايحصلش memory leak أو setState بعد dispose
