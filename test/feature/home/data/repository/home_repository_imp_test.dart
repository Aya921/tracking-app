...
group("test saveDataToLocalStorage in HomeRepositoryImp", () {
  test(
    ...
    () async {
      ...
      verify(mockHomeLocalDataSource.saveDataToLocalStorage(orderList))
          .called(1);
    },
  );
...
group("test getAllSavedOrders in HomeRepositoryImp", () {
  test(
    ...
    () async {
      ...
      when(mockHomeLocalDataSource.getAllSavedOrders())
          .thenAnswer((_) async => mockResult);
...
group("test deleteOrder in HomeRepositoryImp", () {
  test(
    ...
    () async {
      ...
      when(mockHomeLocalDataSource.deleteOrder(orderId))
          .thenAnswer((_) async => mockResult);
...
group("test startOrder in HomeRepositoryImp", () {
  test(
    "when call startOrder with correct orderId it should return SuccessResult<StartOrderResponseEntity>",
    () async {
      // arrange
      const orderId = "123";
      final fakeResponse =StartOrderResponseEntity(id: "1234", user: "19ri2921",
       orderItems:[ StartOrderItemEntity(product: "kefow", price: 123, quantity: 1, id: "2eefwe")],
        totalPrice: 500, paymentType: "cash", isPaid: true, isDelivered: false, state: "outForDelivery", orderNumber: "#123");

      final mockResult = SucessResult<StartOrderResponseEntity>(fakeResponse);
      provideDummy<Result<StartOrderResponseEntity>>(mockResult);

      when(mockHomeRemoteDataSource.startOrder(orderId))
          .thenAnswer((_) async => mockResult);
      ...
      // assert
      expect(res, isA<SucessResult<StartOrderResponseEntity>>());
      final acResult = res as SucessResult<StartOrderResponseEntity>;
  ...
  test(
    ...
    () async {
      ...
      final mockResult = FailedResult<StartOrderResponseEntity>("error");
      provideDummy<Result<StartOrderResponseEntity>>(mockResult);
      ...
      // assert
      expect(res, isA<FailedResult<StartOrderResponseEntity>>());
      final acResult = res as FailedResult<StartOrderResponseEntity>;
      expect(acResult.errorMessage, "error");
      verify(mockHomeRemoteDataSource.startOrder(orderId)).called(1);
    },
  );
...
group("test addDateToRemote in HomeRepositoryImp", () {
  test(
    ...
    () async {
      ...
      when(mockHomeRemoteDataSource.addDateToRemote(remoteDataEntity))
          .thenAnswer((_) async => mockResult);
      ...
      // assert
      expect(res, isA<SucessResult<void>>());
      verify(mockHomeRemoteDataSource.addDateToRemote(remoteDataEntity)).called(1);
    },
  );
  ...
  test(
    ...
    () async {
      ...
      when(mockHomeRemoteDataSource.addDateToRemote(remoteData))
          .thenAnswer((_) async => mockResult);
