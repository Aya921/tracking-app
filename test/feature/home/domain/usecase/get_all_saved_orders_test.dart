...
void main() {
  ...
  late GetAllSavedOrdersUseCase getAllSavedOrdersUseCase;
  ...
  setUpAll(() {
    ...
    getAllSavedOrdersUseCase = GetAllSavedOrdersUseCase(mockHomeRepository);
  ...
  group('GetAllSavedOrdersUseCase Tests', () {
    test(
      'when call getAllSavedOrders and repository returns data it should return SucessResult<List<OrderEntity>>',
      () async {
        // arrange
        ...
        when(
          mockHomeRepository.getAllSavedOrders(),
        ).thenAnswer((_) async => mockResult);

        // act
        final res = await getAllSavedOrdersUseCase.getAllSavedOrders();

        // assert
        expect(res, isA<SucessResult<List<OrderEntity>?>>());
        verify(mockHomeRepository.getAllSavedOrders()).called(1);
      },
    );

    test(
      'when call getAllSavedOrders and repository returns error it should return FailedResult<List<OrderEntity>>',
      () async {
        // arrange
