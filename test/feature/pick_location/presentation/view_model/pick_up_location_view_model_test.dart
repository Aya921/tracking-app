...
final RemoteDataEntity remoteDataEntity=RemoteDataEntity(
    driverEntity:  const DriverEntity(        country: "Egypt",
        firstName: "mariam1",
        lastName: "mohmed2",
        vehicleType: "676b31a45d05310ca82657ac",
        vehicleNumber: "12221",
        vehicleLicense: "17145723-dce1-4b92-b9dd-f07fde1d2a0c-WhatsApp Image 2025-10-12 at 22.27.44.jpeg",
        nid: "12345678912345",
        nidImg: "d423a44e-a08e-4884-bd04-eddcacde34db-WhatsApp Image 2025-10-08 at 15.08.18.jpeg",
    ...
    orderEntity: OrderEntity(id: "",
        ...
        orderItems: [
            ...
            street: "benha", city: "qaliubia",
            phone: "01061728082", lat: "37.7749", long: "-122.4194"),
        store: StoreEntity(
            name: "Elevate FlowerApp Store",
            image: "https://www.elevateegy.com/elevate.png",
            address: "123 Fixed Address, City, Country",
            phoneNumber: "1234567890",
            latLong: "37.7749,-122.4194"), paymentInfoEntity:
        PaymentInfoEntity("cash", "2025-10-13T10:22:15.841Z", true),
        orderInfoEntity: OrderInfoEntity(true,
            "Accepted", "1234", "2025-10-13T10:47:38.239Z", "2025-10-13T10:47:38.239Z",
            12345, 234567)),
...
when(mockLocation.onLocationChanged).thenAnswer((_)=>
    Stream.fromIterable([mockLocationData]));
...
test("init map and set error", ()async{
  ...
  when(mockLocation.serviceEnabled()).thenAnswer((_)async=>true);
  when(mockLocation.hasPermission()).thenAnswer((_)async=>
...
expect(mapViewModel.driverLocation, const LatLng(37.7749, -122.4194));
  ...
  test('_setDriverLocation sets driver location from current location', () async {
    // Arrange
    when(mockLocation.serviceEnabled()).thenAnswer((_)async=>true);
    when(mockLocation.hasPermission()).thenAnswer((_)async=>
    PermissionStatus.granted);
    ...
    when(mockLocation.onLocationChanged).thenAnswer((_)=>
        Stream.fromIterable([mockLocationData]));
    when(mockLocation.enableBackgroundMode(
      enable: true
  ...
  test(" setDriverLocation fallback when service disabled", () async {
    ...
    expectLater(
          () async => await
          mapViewModel.setDriverLocation(remoteDataEntity, true),
      throwsException,
    );
  });
  group('parseLatLng', () {
    ...
    test('throws FormatException when input is empty', () {
      expect(() =>mapViewModel.parseLatLng(''),
          throwsA(isA<FormatException>()));
    });

    test('throws FormatException when input contains non-numeric values', () {
      expect(() => mapViewModel.parseLatLng('abc, def'),
          throwsA(isA<FormatException>()));
    });
